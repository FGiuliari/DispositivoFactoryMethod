...
public class RilevamentoPosizioneTest implements Runnable {
	double lastPosition[];
	double position[] = { 10.991621500000065, 45.43838419999999 };

	public RilevamentoPosizioneTest() {
	...
	@Override
	public void run() {
		System.out.println("avviato il thread per generare le posizioni");
		while (true) {
			generateRandomPosition();
			long time = System.currentTimeMillis();
			new Thread(new sendPositionToServerTest(position, time)).start();
